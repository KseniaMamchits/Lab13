...
namespace lab13
{
    static class MKMLog
    {
        ...
        {
            int Count = 0;
            StreamReader reading = new StreamReader(@"D:\Mpr\MKMlogfile.txt");
        ...
        {
            StreamWriter write = new StreamWriter(@"D:\Mpr\MKMlogfile.txt", true);
        ...
        public static void Delete() // Оставить записи за текущий час
        {
            ...
            StreamReader reading = new StreamReader(@"D:\Mpr\MKMlogfile.txt");
            FileInfo fileInf = new FileInfo(@"D:\Mpr\MKMlogfile.txt");
        ...
        static class MKMFileManager
        {
                ...
                foreach (DriveInfo drive in drives)
                {
                    if (drive.Name.Contains(nameDisk) && drive.IsReady)
                    {
                        ...
                        if (dirInfo.Exists)
                        {
                            ...
                            FileInfo fileInf = new FileInfo(@"D:\Mpr\OOP\MKMInspect\MKMdirinfo.txt");
                            StreamWriter sw = File.CreateText(fileInf.FullName);
                            sw.WriteLine("ИНФОРМАЦИЯ");
                            sw.Close();
                            fileInf.CopyTo(@"D:\Mpr\OOP\MKMInspect\newfile.txt", true);
                            fileInf.Delete();
                        }
                        else
                        {
            ...
            public static void Method2(string path)
            {
                ...
                DirectoryInfo dirInfo = new DirectoryInfo(@"D:\Mpr\OOP");
                dirInfo.CreateSubdirectory("MKMFiles");
                if (!dirInfo.Exists)
                {
                    dirInfo.Create();
                }
                ...
                {
                    item.CopyTo($@"D:\Mpr\OOP\MKMFiles\{item.Name}", true);
                }
                DirectoryInfo dir = new DirectoryInfo(@"D:\Mpr\OOP\MKMInspect\");
                dir.CreateSubdirectory("MKMFiles");
                if (!dir.Exists)
                {
                    dir.Create();
                }
                DirectoryInfo dir2 = new DirectoryInfo(@"D:\Mpr\OOP\MKMFiles\");
                FileInfo[] fileInf2 = dir2.GetFiles();
                foreach (var item in fileInf2)
                {
                    item.CopyTo($@"D:\Mpr\OOP\MKMInspect\MKMFiles\{item.Name}", true);
                }
                dir2.Delete(true);
            }
        }
        class Program
        {
            ...
            {
                MKMDiskInfo.PrintFreePlace("C");
                MKMDiskInfo.FileSystem("D");
                MKMDiskInfo.InfoDrivers();
                MKMFileInfo.Path(@"D:\Mpr\OOP\file1.txt");
                MKMFileInfo.Path(@"D:\Mpr\OOP\file2.txt");
                MKMFileInfo.SRN(@"D:\Mpr\OOP\file3.txt");
                MKMFileInfo.TimeOfCreation(@"D:\Mpr\OOP\file4.txt");
                MKMDirInfo.NumberOfFiles(@"D:\Mpr\OOP");
                MKMDirInfo.TimeOfCreation(@"D:\Mpr\OOP");
                MKMDirInfo.NumberOfDir(@"D:\Mpr\OOP");
                MKMDirInfo.Parent(@"D:\Mpr\OOP");
                MKMFileManager.Method1("D");
                MKMFileManager.Method2(@"D:\Mpr\OOP");
